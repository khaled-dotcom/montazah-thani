...
if (!['build', 'runtime', 'operator', 'all'].includes(requested)) {
  console.error(`Unknown scope "${requested}". Use build, runtime, operator or all.`);
  process.exit(2);
}
 ...
 *
 *   build     the content and the compiled artefact — asserted while building
 *   runtime   what the web container itself holds — asserted before it serves
 *   operator  secrets split across services by docker-compose.yml, which no
 *             single container can see. Only the host, with the whole .env in
 *             front of it, can answer these; asserting them inside a container
 *             would turn the least-privilege split into a restart loop.
 */
...
/** Loopback, or a bare service name that only resolves on a private network. */
function privateDatabaseHost(url) {
  let hostname;
  try {
...
check({
  name: 'DATABASE_URL names a Postgres database',
  scope: 'runtime',
  ok: /^postgres(ql)?:\/\//.test(databaseUrl),
  fix: 'Set DATABASE_URL to the site\'s Postgres connection string. Without it every booking, every contact message and the whole staff dashboard fail — the public pages would still serve, which is exactly what makes this worth failing loudly over.',
});

/* The agent keeps its own `appointments` table in `districts_db`. Pointed at
   that database the site would find it, decide the schema already exists, and
   then read and write the assistant's bookings as if they were its own. Two
   systems silently sharing one table is not a thing that degrades gracefully. */
check({
  name: 'The site has its own database, separate from the assistant',
  scope: 'runtime',
  ok: databaseUrl === '' || !/\/districts_db(\?|$)/.test(databaseUrl),
  fix: 'DATABASE_URL points at districts_db, which belongs to the assistant and already has an `appointments` table of its own. Give the site its own database (montazah_site) so the two cannot overwrite each other.',
});

check({
  name: 'The database connection is not sent in the clear',
  scope: 'runtime',
  /* Same rule as lib/sql.ts, deliberately: loopback, or a single-label
     hostname like `db`, which cannot be a public DNS name and therefore never
     leaves the host. If these two disagree, one of them lets plaintext through
     while the other refuses to connect at all. */
  ok:
    databaseUrl === '' ||
    /sslmode=(require|verify-ca|verify-full)/.test(databaseUrl) ||
    privateDatabaseHost(databaseUrl),
  fix: 'The database is on another host and the connection does not require TLS. Residents\' names and telephone numbers would cross the network unencrypted. Add ?sslmode=require, or keep the database on the internal compose network.',
});
...
if (agentUrl) {
  ...
  /* Reachability and authentication are different questions, and the check
     above only answers the first. An assistant on a public ingress is one
     `curl` away from filing complaints in residents' names — ALLOWED_ORIGINS
     does not stop that, because CORS binds browsers and nothing else. Behind a
     private network the secret is unnecessary, so it is only demanded when the
     host is genuinely public. */
  check({
    name: 'Assistant on a public host is behind a shared secret',
    scope: 'operator',
    ok: privateDatabaseHost(agentUrl) || (process.env.AGENT_TOKEN ?? '').length >= 32,
    fix: 'AGENT_URL is a public address, so set AGENT_TOKEN to at least 32 random characters on BOTH services — the site sends it as a bearer token and the assistant refuses /api/chat without it. Generate: python -c "import secrets; print(secrets.token_hex(32))"',
  });
  ...
  check({
    ...
    ok: exists('agent/site_knowledge/montazah-thani.json'),
...
const heading = { build: 'build', runtime: 'runtime', all: 'full' }[requested];
console.log(`
Production preflight (${heading}) — حي المنتزه الثانية
` + '─'.repeat(72));
...
if (failures.length) {
  ...
  const verdict = requested === 'runtime' ? 'Clear to serve.' : 'Clear to build.';
  console.log(`
All ${passes.length} checks passed. ${verdict}
`);
}
