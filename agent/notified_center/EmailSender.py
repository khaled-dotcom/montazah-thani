...
class EmailClient:
    ...
    # =========================
    # SMTP Send — arbitrary recipient
    # =========================
    def send_to(self, recipients, subject, body, html: str = None) -> bool:
        """
        Sends to explicit recipients (used for citizen notifications).
        Returns True on success. Never raises — a failed notification must not
        ...
        if not sender or not password:
            logger.warning("[NotificationCenter] Email not configured; skipping send.")
            return False
        ...
        try:
            with smtplib.SMTP_SSL(self.smtp_server, self.smtp_port, timeout=20) as server:
                ...
                for receiver in recipients:
                    ...
                    msg["From"] = os.environ.get("NOTIFICATION_EMAIL_FROM_NAME", sender)
                    msg["To"] = receiver
                    msg["Subject"] = subject
            ...
            logger.info("[NotificationCenter] Sent '%s' to %s", subject, recipients)
            return True
        except Exception as e:
            logger.error("[NotificationCenter] Failed sending '%s': %s", subject, e)
            return False

    # =========================
    # SMTP Send — admin alerts
    # =========================
    def send_email(self, subject, body):
        ...
        machine_identifier = os.environ.get("SERVER_IDENTIFIER", "alx-districts-agent")
...
def send_production_alert(subject: str, body_or_error, context: dict = None):
    ...
    Included in context: session_id, district_id, reference_id, etc.
    """
