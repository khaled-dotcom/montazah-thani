...
رسالة المواطن
     │
     ├─ بياناته مسجّلة؟ ──لأ──→  intake        بوابة التسجيل
     │                            (بتتخطى تصنيف النية بالكامل)
     أه
     ↓
   intent  ─┬─ inquiry     → rag →  رد من قاعدة المعرفة
            ├─ appointment → rag →  حجز + بطاقة PNG
            ├─ complaint   ──────→  بلاغ + تحويل لإدارة
...
الفصل ده هو اللي بيخلي المواطن يقدّم بلاغ وبعده بدقيقة يحجز موعد **من غير ما يتسأل على بياناته تاني**.
...
graph/                      عقل الوكيل (LangGraph)
  ...
  nodes/intake_node.py      بوابة تسجيل بيانات المواطن
  nodes/intent_node.py      تصنيف النية + توليد استعلامات البحث
  nodes/complaint_node.py   تسجيل البلاغات
  nodes/appointment_node.py حجز المواعيد + البطاقة
